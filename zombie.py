...
class Zombie:
    ...
    def set_random_location(self):
        self.tx, self.ty = random.randint(100, 1280 - 100), random.randint(
            100, 1024 - 100
        )
        return BehaviorTree.SUCCESS
    ...
    def is_more_ball_than_boy(self):
    ...
    def build_behavior_tree(self):
        ...
        a4 = Action("도주", self.run_from_boy)

        a5 = Action("접근", self.move_to_boy)
        c2 = Condition("소년보다 공이 많은가?", self.is_more_ball_than_boy)
        root = SEQ_chase_boy = Sequence("추적", c2, a5)

        root = SEL_chase_or_run = Selector("추적 또는 도주", SEQ_chase_boy, a4)

        c1 = Condition("소년이 근처에 있는가?", self.is_boy_nearby, 7)
        root = SEQ_find_boy = Sequence("소년발견", c1, SEL_chase_or_run)

        root = SEL_chase_or_flee = Selector("추적 또는 배회", SEQ_find_boy, SEQ_wander)
